...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument(
        ...
        help="Lineage registry JSON (default: %%(default)s).",
    )
    ap.add_argument(
        "--timeseries",
        default="data/out/02_lineage_tracking/lineage_timeseries.csv",
        help="Lineage timeseries CSV for quarter list (default: %%(default)s).",
    )
    ap.add_argument(
        "--quarterly-embeddings",
        default="data/out/experiments/stage1_quarterly_embeddings/quarterly_embeddings.npz",
        help="Quarterly SciBERT embeddings NPZ (default: %%(default)s). "
             "If missing, the semantic channel is skipped.",
    )
    ap.add_argument(
        "--partitions-dir",
        default="data/out/cache_cum/partitions_cum",
        help="Directory with partition JSON files (default: %%(default)s).",
    )
    ap.add_argument(
        "--slices-dir",
        default="data/current_ingest/slices",
        help="Directory with quarterly parquet slices (default: %%(default)s).",
    )
    ap.add_argument(
        "--out",
        default="data/out/02_lineage_tracking/convergence_features.csv",
...
def main() -> None:
    ...
    quarterly_embeddings: Optional[Dict[str, Dict[int, np.ndarray]]] = None
    ...
    prev_semantic_sim: Optional[Dict[int, List[Tuple[int, float]]]] = None
